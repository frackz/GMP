...
module.exports = {
	...
	async execute(client, interaction, config, db) {
        ...
        if (report == null) {return interaction.reply({content: "The report doens't exist.", ephemeral: true})}
        const res = await axios({url: 'https://api.mojang.com/user/profiles/'+target+'/names',method: 'GET'});
        await interaction.reply({content: "The report has been declined!", ephemeral: true})
        ...
        try {
            userg.send({
                embeds:[{
                    ...
                    "description": "Your report against **"+name+"** has been declined!",
                    "color": 16774400
                }]
            })
        } catch {
